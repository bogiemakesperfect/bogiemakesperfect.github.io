...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email_err) && empty($password_err)) {
        ...
        $sql = "SELECT musteriler_id,musteriler_eposta,musteriler_sifre FROM viento_musteriler WHERE musteriler_eposta = ?";
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if (mysqli_stmt_num_rows($stmt) == 1) {
                    ...
                    if (mysqli_stmt_fetch($stmt)) {
                        var_dump($hashed_password, $password, password_verify($password, $hashed_password));
...
<head>
    ...
    <title><?php echo $language["Login"]; ?> | Viento</title>
                    ...
                    <div class="card">
                        <div class="card-body p-4">
                            <div class="text-center mt-2">
                                <h5 class="text-primary">Kullanıcı Paneline Hoşgeldiniz</h5>
                                <p class="text-muted">Devam Etmek İçin Giriş Yapın</p>
                            </div>
                            <div class="p-2 mt-4">
                                <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">

                                    <div class="mb-3 <?php echo (!empty($email_err)) ? 'has-error' : ''; ?>">
                                        <label class="form-label" for="email">Eposta</label>
                                        <input type="text" class="form-control" id="email" name="email" placeholder="Eposta Adresinizi Girin" value="">
                                    ...
                                    <div class="mt-4 text-center">
                                        <p class="mb-0">Hesabın Yok mu? <a href="auth-register.php" class="fw-medium text-primary"> Kayıt Ol </a> </p>
                                    </div>
                                </form>
                            </div>
