...
if (isset($_POST['onayla'])) {
    ...
    if ($res->num_rows == 0) {
        ...
        if (empty($_POST['musteriler_isim']) || empty($_POST['billing-address']) || empty($_POST['billing-email']) || empty($_POST['billing-phone']) || empty($_POST['siparisler_sehir']) || empty($_POST['billing-ilce']) || empty($_POST['zip-code'])) {
            ...
            $odemekodu = uniqid();
            $musterilerid = $_SESSION['customer_id'];
            ...
            // Sepeti temizle
            $deleteQuery = "DELETE FROM viento_sepet WHERE musteriler_id = ?";
            $deleteStmt = $link->prepare($deleteQuery);
            $deleteStmt->bind_param("i", $_SESSION['customer_id']);
            $deleteStmt->execute();
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.btn-add-to-cart').click(function(e) {
                ...
                $.ajax({
                    url: '/odeme/Customer/sepet-kontrol.php', // Ürünlerin bulunduğu sepeti kontrol eden PHP dosyasının yolunu buraya yazın
                    method: 'POST',
                    data: {
                        urun_id: urun_id
                    },
                    success: function(response) {
                        ...
                        if (response === 'true') {
                            ...
                            $.ajax({
                                url: '/odeme/Customer/sepete-ekle.php', // Sepete ürün ekleme işlemini gerçekleştiren PHP dosyasının yolunu buraya yazın
                                method: 'POST',
                                data: {
                                    urun_id: urun_id,
            ...
            function updateCart() {
                $.ajax({
                    url: '/odeme/Customer/sepet-guncelle.php',
                    method: 'GET',
                    success: function(response) {
                                                ...
                                                <div class="row">
                                                    <div class="col-lg-4">
                                                        <div class="mb-4 mb-lg-0">
                                                            ...
                                                            <select class="form-control form-select" title="Country" name="siparisler_sehir" id="siparisler_sehir">
                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-centered mb-0 table-nowrap" id="cart-summary">
                                        ...
                                        <tbody>
                                            <?php
                                            $query = "SELECT * FROM viento_sepet INNER JOIN viento_urunler ON viento_sepet.urunler_id = viento_urunler.urunler_id WHERE viento_sepet.musteriler_id = '" . $_SESSION['customer_id'] . "'";
                                            $stmt = $link->prepare($query);
                                            $stmt->execute();
                                            $res = $stmt->get_result();
                                            while ($prod = $res->fetch_object()) {
